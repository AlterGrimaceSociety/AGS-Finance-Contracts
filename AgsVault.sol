...
// File: contracts/interfaces/IMasterGrimace.sol
interface IMasterGrimace {
...
contract AgsVault is Ownable, Pausable {
    ...
    IMasterGrimace public immutable masterchef;
    ...
    function harvest() external notContract whenNotPaused {
        IMasterGrimace(masterchef).leaveStaking(0);
    ...
    function emergencyWithdraw() external onlyAdmin {
        IMasterGrimace(masterchef).emergencyWithdraw(0);
    }
    ...
    function calculateHarvestCakeRewards() external view returns (uint256) {
        uint256 amount = IMasterGrimace(masterchef).pendingAgs(0, address(this));
    ...
    function withdraw(uint256 _shares) public notContract {
        ...
        if (bal < currentAmount) {
            ...
            IMasterGrimace(masterchef).leaveStaking(balWithdraw);
    ...
    function balanceOf() public view returns (uint256) {
        (uint256 amount, ) = IMasterGrimace(masterchef).userInfo(0, address(this));
    ...
    function _earn() internal {
        uint256 bal = available();
        if (bal > 0) {
            IMasterGrimace(masterchef).enterStaking(bal);
        }
    }
